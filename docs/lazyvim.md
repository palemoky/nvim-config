...
- Buffer：文件在内存中的表示，每一个文件，在内存中都对应一个 Buffer。Buffer 是内存全局共享的，它本身不决定怎么在屏幕上显示，只是单纯地装载着文件的内容。

- Window：是 Buffer 的**视口**，通过分屏（如 `:vsp` 左右分屏、`:sp` 上下分屏）时，就创建了多个 Window。一个 Window 只能显示一个 Buffer；但**同一个 Buffer 可以同时在多个 Window 中显示**

- Tab：Window 的容器，一个 Tab 中可以包含任意数量的分屏 Window。切换 Tab，本质上是切换了一整套不同的分屏布局。Tab 可以实现会话的保持与恢复（`<leader>qs`）、多实例复用 LSP、共享寄存器、宏等
...
插件用的是默认按键前缀 `<leader>t`，配置里只自定义了 `table_mode_corner = "|"`（markdown 兼容的表格边角）。

**创建表格**

- `<leader>tm` 开启 Table Mode 后，在行首打 `|` 会自动生成/对齐表格边框，每加一个 `|` 都会实时重排。
- 已有分隔符文本（比如逗号分隔）转表格：选中后 `<leader>tt`（用默认分隔符 `,`），或 `<leader>T` 会提示输入自定义分隔符再执行 `:Tableize`。
- `||`（两个竖线）在表头行下方展开成表头分隔线 `|---|---|`。
...
开着 Table Mode 时会自动重新对齐。

**移动列**：同样没有原生"移动列"命令，只有插入/删除：

| 键             | 作用                                   |
| -------------- | -------------------------------------- |
| `<leader>tiC`  | 在光标所在列**前**插入新列             |
| `<leader>tic`  | 在光标所在列**后**插入新列             |
| `<leader>tdc`  | 删除光标所在列（前面加数字可删多列）   |
| `<leader>tdd`  | 删除当前行（表格行），支持 `[count]tdd` 删多行 |

要"挪列"只能插入空列，再用单元格文本对象 `di|`/`ci|`/`yi|`（inner cell）、`da|`/`ca|`（around cell，含分隔符）把内容剪切粘贴过去。

**其他**

- `<leader>tr` 或 `:TableModeRealign`：手动重新对齐
- `<leader>ts`：按光标所在列排序（`:TableSort`）
...
```text
标题 Hello 世界         → 标题 Hello 世界
这是 test 测试 123 混排   → 这是 test 测试 123 混排
```
...
由插件 [`im-select.nvim`](https://github.com/keaising/im-select.nvim)（底层调用 `macism` 执行切换）完成，配置在 `macism.lua`。
